...
public class LabyrinthTrials : UniversalFileWrapper<LabyrinthTrial>
{
	...
	{
		return EntriesList.FirstOrDefault((LabyrinthTrial x) => x.Area.Id == id);
	}

	public LabyrinthTrial GetLabyrinthTrialById(int index)
	{
		return EntriesList.FirstOrDefault((LabyrinthTrial x) => x.Id == index);
	}

	public LabyrinthTrial GetLabyrinthTrialByArea(WorldArea area)
	{
		return EntriesList.FirstOrDefault((LabyrinthTrial x) => object.Equals(x.Area, area));
	}
